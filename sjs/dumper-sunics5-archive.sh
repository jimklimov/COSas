...
# dumper-sunics5-archive.sh
# (C) Nov 2007-Dec 2008 by Jim Klimov, COS&HT
# $Id: dumper-sunics5-archive.sh,v 1.23 2012/05/06 15:56:19 jim Exp $
...
# For use from cron
# 45 3 * * * [ -x /opt/COSas/bin/dumper-sunics5-archive.sh ] && /opt/COSas/bin/dumper-sunics5-archive.sh
...
TS=`/bin/date '+%Y%m%dT%H%M%SZ'` || TS='last'   ### NON-UTC!
[ x"$extTS" != x ] && TS="$extTS"
[ x"$REMOVEORIG" = x ] && REMOVEORIG=Y
[ x"$REMOVELOCAL" = x ] && REMOVELOCAL=N
TSDIR=`echo "$TS" | /bin/cut -c 1-8`
BASEDIR=/var/opt/SUNWics5/csdb/archive
ARCHFILE="$BASEDIR"/`hostname`.`domainname`_csdb-archive-"$TS".tar
DUMPDIR="/DUMP/SUNWics5/csdb/archive"
...
if [ -f "$LOCK" ]; then
    ...
    if [ x"$TRYOLDPID" != x ]; then
        ...
        echo "= SUNWics archive dump aborted because another copy is running - lockfile found:
$LF
Aborting..." | wall
        exit 1
    fi
